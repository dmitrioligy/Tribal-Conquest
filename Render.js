function Render(game, socket) 
{
    ...
    for(var z = 0; z < game.Player_List.length; ++z)
        if(window.name == game.Player_List[z].Name)
            playerIndex = z;

    var orientation;
    switch(game.Player_List.length)
    {
        case 2:
    ...
    function makePizza(i) 
    {
        return function (context) {
            ...
            context.lineTo(OX + 2 * radius * Math.cos(i * Math.PI / 4 + orientation), OY + 2 * radius * Math.sin(i * Math.PI / 4 + orientation));
            context.arc(OX, OY, 2 * radius, i * Math.PI / 4 + orientation, (i + 1) * Math.PI / 4 + orientation);
            context.closePath();
    ...
    function makeCell(i, j) 
    {
        return function (context) {
            ...
            context.beginPath();
            context.moveTo(OX + r1 * Math.cos((j + 1) * Math.PI / 12 + orientation), OY + r1 * Math.sin((j + 1) * Math.PI / 12 + orientation));
            context.arc(OX, OY, r1, (j + 1) * Math.PI / 12 + orientation, j * Math.PI / 12 + orientation, true);
            context.lineTo(OX + r2 * Math.cos(j * Math.PI / 12 + orientation), OY + r2 * Math.sin(j * Math.PI / 12 + orientation));
